...
-- SQL QUERY TO FIND THE 5TH HIGHEST WITHDRAWAL AMOUNT OF EACH YEAR.
   ...
   WITH HIGHEST_TRANSACTIONS_OF_THE_YEAR AS(
      ...
      REGEXP_LIKE(TRIM(' ' FROM REPLACE(WITHDRAWAL_AMT, '"','')), &VAR)
   ), 
   FIFTH_HIGHEST_TRANSACTION_OF_THE_YEAR AS (
      SELECT YEAR, WITHDRAWAL_AMT, ROW_NUMBER() OVER (PARTITION BY YEAR ORDER BY WITHDRAWAL_AMT DESC) 
      AS FIFTH_H_T
      FROM HIGHEST_TRANSACTIONS_OF_THE_YEAR
   ) 
   SELECT YEAR, WITHDRAWAL_AMT AS FIFTH_HIGHEST_WITHDRAWAL_AMOUNT_OF_THE_YEAR 
   FROM FIFTH_HIGHEST_TRANSACTION_OF_THE_YEAR
...
-- SQL QUERY TO FIND THE FIRST FIVE LARGEST WITHDRAWAL TRANSACTIONS OCCURED IN THE YEAR 2018 
   ...
   AS FIRST_FIVE_LARGEST_WITHDRAWAL_TRANSACTIONS_OCCURED_IN_THE_YEAR_2018 FROM BANK_TRANSACTION 
...
-- SQL QUERY TO COUNT THE WITHDRAWAL TRANSACTIONS BETWEEN MAY 5,2018 AND MAR 7,2019 
   ...
   SELECT COUNT(WITHDRAWAL_AMT) AS TOTAL_COUNT_OF_WITHDRAWAL_AMOUNT FROM BANK_TRANSACTION 
 ...
 -- SOME VALUES OF WITHDRAWAL_AMT ARE DATES. 
    SELECT WITHDRAWAL_AMT FROM BANK_TRANSACTION WHERE WITHDRAWAL_AMT='23-Jun-15'; 
 ...
 -- TO GET RID OF THOSE DATES IN WITHDRAWAL_AMT, IAM USING REGEXP_LIKE FUNCTION.
    ...
    AND REGEXP_LIKE(REPLACE(WITHDRAWAL_AMT,' ',''), &VAR); 
